...
describe('dsh-feishu-remote loader contract', () => {
  ...
  it('keeps the keyed settings slot contract used by 0.1.1-rc.2', () => {
    ...
    expect(client).toContain('id: "feishu-remote"')
    expect(client).toContain('"page.add": "添加机器人"')
    expect(client).toContain('const converted = await props.convertLegacy()')
    expect(client).not.toContain('"确认转换"')
    expect(client).not.toContain('"转换为多机器人配置"')
