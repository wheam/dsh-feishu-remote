...
| 5 | **群范围 fail-open**：只有 open_id 白名单，owner 可在机器人所在任意群执行并向该群公开输出（@ 是投递条件不是授权条件） | 当时采纳 `allowedChatIds` fail-closed；2026-08-22 改为群范围默认不限、非空列表可选收窄；**2026-08-25 再由产品决策移除 open_id 操作者白名单，任何用户均可发起任务** | 05 §2.9/§5.3/§6、01 P0.3、03 D5 |
...
| 20 | open_id 自举路径缺失（CLI 向导已删、设置卡片 P1） | 已以 Host 本机 PersonalAgent 扫码绑定替代；手工部署从飞书管理端取 ID，拒绝日志只显示脱敏后缀 | 05 §2.9 |
