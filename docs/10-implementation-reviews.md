...
## Round 12（2026-08-19，流式卡片提交 0680644 独立 review）

结论：**APPROVE with P1 fixes** → 修复后关闭。2 项 P1、4 项 P2、1 项 NIT，修复对照：

| # | 级别 | 问题 | 处置 |
| --- | --- | --- | --- |
| F1 | P1 | `message.patch` + `streaming_mode` 不是官方契约化的 token 级打字机（该契约属 cardkit 实体 + `cardElement.content`）；docs 声称"打字机效果上屏"无契约支撑 | **修（文档口径）**：README/docs-05 改为"与参考实现同路径的 600ms 整卡增量刷新，客户端视觉行为以真实租户验收为准"；cardkit 元素级流式保留为可选升级；docs/09 §7 增验收项 |
| F2 | P1 | 600ms × 多会话可能压过全局 5 QPS；sendChain 串行下排队进度更新拖累终态（"终态优先"不完整） | **修（代码）**：turn/end 后链上排队的非终态进度更新一律跳过（门禁 + 运行期双检查）；`/view` 走显式重渲染不受影响；docs 记录多会话节奏自限速行为。注：复算证实积压有界（链式自限速），非无界增长 |
| F3 | P2 | 流式模式中点击停止按钮处于官方"回调后无法立即更新"窗口，无官方保证 | **收窄记录**：现有 transient 重试 + 新终态卡兜底可覆盖；真实租户验收项（docs/09 §7 第 7 条） |
| F4 | P2 | 运行期 oversize 兜底卡标题误显示"已完成"且无停止按钮 | **修（代码）**：`buildOversizeCard` 增 `'running'` 态（⏳ 生成中 + 停止按钮 + streaming:true），终态四枚举保持 false |
| F5 | P2 | 永久错误分类缺官方 patch 接口的 230011/230110/230013/230027/232009（白等 8-10s 重试并拖住全局调度器） | **修（代码）**：五码加入 permanent 分类（官方 patch 接口错误码表逐条核实），测试补断言 |
| F6 | P2 | 新增测试只验 JSON 形状；未覆盖 live SEND→live PATCH(true)→terminal PATCH(false) 完整序列、四 outcome 参数化、oversize、陈旧更新跳过 | **修（测试）**：bridge +2（同 messageId 完整序列；cardGate 阻塞下 turn/end 跳过陈旧更新 + 结算后 /view 显式重渲染）、cards +2（四 outcome 流式关闭；oversize 运行/终态） |
| F7 | NIT | 文档测试计数口径错误：父提交实际 104（非 102），本次 +4 → 108 | **修（文档）**：docs/05 计数改为 104/112（实际以 vitest 为准） |

残余风险（如实记录）：流式卡片的客户端视觉行为、流式中停止按钮交互均需真实
租户验收（docs/09 §7）；多会话（≥4 繁忙话题）下卡片节奏退化为全局 5 QPS 均摊，
属已知取舍，keyed rate-limit 留作后续。
