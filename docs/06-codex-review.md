...
> 历史审查记录。有关个人操作者白名单的建议已被 2026-08-25 产品决策明确覆盖：当前实现固定
> 对所有用户开放，仅保留可选的 `allowedChatIds` 群范围；下文相关条目用于说明设计演进。
...
> **后续产品决策（2026-08-25）**：明确覆盖本条建议，操作者固定对所有人开放；旧
> `allowedOpenIds` / `allowAllUsers` 只兼容读取且运行时忽略。本节保留为历史审查记录。
...
> **后续产品决策（2026-08-22、2026-08-25）**：接受机器人在所在任意群执行并向该群公开输出；
> `allowedChatIds` 为空表示不限群，非空列表作为可选部署范围限制；2026-08-25 又移除
> `allowedOpenIds` 操作者限制。2026-08-22 已实现话题首次 @后持久化激活：首次 @回填前文，
