...
> 消息但每轮必须由任意用户明确 @，触发时读取全群有界历史；私聊按更紧预算有界回溯。
...
任意用户可在机器人加入的任意群触发读取；需要收窄范围时可配置非空 `allowedChatIds`
...
- **F8 群历史 = 不可信输入，按注入面设防。**（Codex F-04 修正）拉取发生在已过发送者
  可选群范围限制之后（复用现有 gate），但**任何群成员**都能写入被回填的历史——
  除 F2 的 JSON 帧与转义外：
...
4. 普通群多人连续发言且不 @ → 机器人静默、零拉取、零 Agent；任意用户 @后读取所有成员的
...
10. `allowedChatIds` 限定范围外的群 @机器人：拒绝逻辑不变，且**无任何历史拉取**（日志确认）。
